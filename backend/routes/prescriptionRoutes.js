...
// Upload Prescription
router.post("/upload", authorizeRoles("user"), upload.single("prescription"), async (req, res) => {
  if (!req.file) {
  ...
  try {
    const fileUrl = `https://${process.env.DOMAIN}/uploads/${req.file.filename}`;
    ...
    res.status(200).json({ success: true, message: "Prescription uploaded successfully!", fileUrl });
  } catch (error) {
